...
//using System.Windows.Media;
using PokemonGo.rocketAPI.WPF.Properties;
...
namespace PokemonGo.rocketAPI.WPF
{
    ...
    public partial class MainWindow : Window
    {
        ...
        //List<GMapMarker> searchAreaOverlay = new List<GMapMarker>();
        //List<GMapMarker> pokestopsOverlay = new List<GMapMarker>();
        //List<GMapMarker> pokemonsOverlay = new List<GMapMarker>();
        //List<GMapMarker> playerOverlay = new List<GMapMarker>();
        ...
        public MainWindow()
        {
            ...
            //MediaTypeNames.Text += " v" + Assembly.GetExecutingAssembly().GetName().Version;
            //CenterToScreen();
        }
        ...
        private void MainWindow_Load(object sender, RoutedEventArgs e)
        {
            ...
            gMapControl1.DragButton = System.Windows.Input.MouseButton.Left;

            gMapControl1.MinZoom = 1;
            gMapControl1.MaxZoom = 20;
            gMapControl1.Zoom = 15;

            //foreach (GMapMarker gMapMarker in searchAreaOverlay)
            //    gMapControl1.Markers.Add(gMapMarker);
            //gMapControl1.Markers.Add(pokestopsOverlay);
            //gMapControl1.Markers.Add(pokemonsOverlay);
            //gMapControl1.Markers.Add(playerOverlay);

            playerMarker =
                new GMapMarker(new PointLatLng(ClientSettings.DefaultLatitude, ClientSettings.DefaultLongitude));
            gMapControl1.Markers.Add(playerMarker);
        ...
        public void CheckVersion()
        {
            try
            {
                ...
                ColoredConsoleWrite(Brushes.Green, "Your version is " + Assembly.GetExecutingAssembly().GetName().Version);
                ColoredConsoleWrite(Brushes.Green, "Github version is " + gitVersion);
                ColoredConsoleWrite(Brushes.Green, "You can find it at www.GitHub.com/1461748123/Pokemon-Go-Rocket-API/releases");
            }
            catch (Exception)
            {
                ColoredConsoleWrite(Brushes.Red, "Unable to check for updates now...");
            }
        }
        ...
        public static void ColoredConsoleWrite(Brush color, string text)
        {
            if (!Instance.Dispatcher.CheckAccess())
            {
                Instance.Dispatcher.Invoke(new Action<Brush, string>(ColoredConsoleWrite), color, text);
                return;
            }
            ...
            Instance.logTextBox.SelectionBrush = color;
        ...
        public void ConsoleClear()
        {
            if (!Instance.Dispatcher.CheckAccess())
            {
                Dispatcher.Invoke(new Action(ConsoleClear));
                return;
            }
        ...
        public void SetStatusText(string text)
        {
            if (!Instance.Dispatcher.CheckAccess())
            {
                Dispatcher.Invoke(new Action<string>(SetStatusText), text);
                return;
            }
        ...
        */
        ...
        private async Task EvolveAllGivenPokemons(Client client, IEnumerable<PokemonData> pokemonToEvolve)
        {
            foreach (var pokemon in pokemonToEvolve)
            {
                ...
                if (countOfEvolvedUnits > 0)
                    ColoredConsoleWrite(Brushes.Cyan,
        ...
        private async void Execute()
        {
            ...
            try
            {
                ...
                {
                    case AuthType.Ptc:
                        ColoredConsoleWrite(Brushes.Green, "Login Type: Pokemon Trainers Club");
                        break;
                    case AuthType.Google:
                        ColoredConsoleWrite(Brushes.Green, "Login Type: Google");
                        break;
                }
                ...
                ColoredConsoleWrite(Brushes.Yellow, "----------------------------");
                ...
                string lat2 = Convert.ToString(ClientSettings.DefaultLatitude);
                string longit2 = Convert.ToString(ClientSettings.DefaultLongitude);
                ColoredConsoleWrite(Brushes.DarkGray, "Name: " + profile.Profile.Username);
                ColoredConsoleWrite(Brushes.DarkGray, "Team: " + profile.Profile.Team);
                if (profile.Profile.Currency.ToArray()[0].Amount > 0) // If player has any pokecoins it will show how many they have.
                    ColoredConsoleWrite(Brushes.DarkGray, "Pokecoins: " + profile.Profile.Currency.ToArray()[0].Amount);
                ColoredConsoleWrite(Brushes.DarkGray, "Stardust: " + profile.Profile.Currency.ToArray()[1].Amount + "\n");
                ColoredConsoleWrite(Brushes.DarkGray, "Latitude: " + ClientSettings.DefaultLatitude);
                ColoredConsoleWrite(Brushes.DarkGray, "Longitude: " + ClientSettings.DefaultLongitude);
                try
                {
                    ColoredConsoleWrite(Brushes.DarkGray, "Country: " + CallAPI("country", lat2.Replace(',', '.'), longit2.Replace(',', '.')));
                    ColoredConsoleWrite(Brushes.DarkGray, "Area: " + CallAPI("place", lat2.Replace(',', '.'), longit2.Replace(',', '.')));
                }
                catch (Exception)
                {
                    ColoredConsoleWrite(Brushes.DarkGray, "Unable to get Country/Place");
                }
                ...
                switch (ClientSettings.TransferType)
                {
                    ...
                    case "IV":
                        ...
                        break;
                    default:
                        ColoredConsoleWrite(Brushes.DarkGray, "Transfering pokemon disabled");
                        break;
                }
                ...
                if (!Stopping)
                {
                    ColoredConsoleWrite(Brushes.Red, $"No nearby useful locations found. Please wait 10 seconds.");
            ...
            catch (TaskCanceledException) { ColoredConsoleWrite(Brushes.Red, "Task Canceled Exception - Restarting"); }
            catch (UriFormatException) { ColoredConsoleWrite(Brushes.Red, "System URI Format Exception - Restarting"); }
            catch (ArgumentOutOfRangeException) { ColoredConsoleWrite(Brushes.Red, "ArgumentOutOfRangeException - Restarting"); }
            catch (ArgumentNullException) { ColoredConsoleWrite(Brushes.Red, "Argument Null Refference - Restarting"); }
            catch (NullReferenceException) { ColoredConsoleWrite(Brushes.Red, "Null Refference - Restarting"); }
            catch (Exception ex) { ColoredConsoleWrite(Brushes.Red, ex.ToString()); }
            finally
            {
                if (!Stopping)
                {
        ...
        private async Task ExecuteCatchAllNearbyPokemons(Client client)
        {
            ...
            foreach (var pokemon in pokemons)
            {
                ...
                ColoredConsoleWrite(Brushes.Green, $"Encounter a {pokemonName} with {pokemonCP} CP and {pokemonIV}% IV");
                do
                {
                ...
                if (caughtPokemonResponse.Status == CatchPokemonResponse.Types.CatchStatus.CatchSuccess)
                {
                    ...
                    ColoredConsoleWrite(Brushes.Red, $"{pokemonName} with {pokemonCP} CP and {pokemonIV}% IV got away..");
        ...
        private void UpdatePlayerLocation(double latitude, double longitude)
        {
            ...
            ColoredConsoleWrite(Brushes.Gray, $"Moving player location to Lat: {latitude}, Lng: {longitude}");
        }
        ...
        private void UpdateMap()
        {
            synchronizationContext.Post(o =>
            {
                ...
                if (wildPokemons != null)
                {
                    ...
                    {
                        //var pokemonMarker = new GMapMarker(new PointLatLng(pokemon.Latitude, pokemon.Longitude),
        ...
        private async Task ExecuteFarmingPokestopsAndPokemons(Client client)
        {
            ...
            {
                ColoredConsoleWrite(Brushes.Red, $"No PokeStops to visit here, please stop the bot and change your location.");
                return;
            }
            pokeStops = rawPokeStops;
            UpdateMap();
            ColoredConsoleWrite(Brushes.Cyan, $"Finding fastest route through all PokeStops..");
            LatLong startingLatLong = new LatLong(ClientSettings.DefaultLatitude, ClientSettings.DefaultLongitude);
            pokeStops = RouteOptimizer.Optimize(rawPokeStops, startingLatLong);
            wildPokemons = mapObjects.MapCells.SelectMany(i => i.WildPokemons);
            if (!ForceUnbanning && !Stopping)
                ColoredConsoleWrite(Brushes.Cyan, $"Visiting {pokeStops.Count()} PokeStops");
            ...
            foreach (var pokeStop in pokeStops)
            {
                ...
                ColoredConsoleWrite(Brushes.Cyan, PokeStopOutput.ToString());
        ...
        private async Task ForceUnban(Client client)
        {
            if (!ForceUnbanning && !Stopping)
            {
                ColoredConsoleWrite(Brushes.LightGreen, "Waiting for last farming action to be complete...");
                ForceUnbanning = true;
                ...
                ColoredConsoleWrite(Brushes.LightGreen, "Starting force unban...");
                ...
                foreach (var pokeStop in pokeStops)
                {
                    if (pokeStop.CooldownCompleteTimestampMs < DateTime.UtcNow.ToUnixTime())
                    {
                        ...
                        if (fortInfo.Name != string.Empty)
                        {
                            ColoredConsoleWrite(Brushes.LightGreen, "Chosen PokeStop " + fortInfo.Name + " for force unban");
                            for (int i = 1; i <= 50; i++)
                            {
                                var fortSearch = await client.SearchFort(pokeStop.Id, pokeStop.Latitude, pokeStop.Longitude);
                                if (fortSearch.ExperienceAwarded == 0)
                                {
                                    ColoredConsoleWrite(Brushes.LightGreen, "Attempt: " + i);
                                }
                                else
                                {
                                    ColoredConsoleWrite(Brushes.LightGreen, "Fuck yes, you are now unbanned! Total attempts: " + i);
                                    done = true;
                                    break;
                                }
                            }
                ...
                if (!done)
                    ColoredConsoleWrite(Brushes.LightGreen, "Force unban failed, please try again.");
                ForceUnbanning = false;
            }
            else
            {
                ColoredConsoleWrite(Brushes.Red, "A action is in play... Please wait.");
            }
        ...
        private async Task TransferAllGivenPokemons(Client client, IEnumerable<PokemonData> unwantedPokemons, float keepPerfectPokemonLimit = 80.0f)
        {
            foreach (var pokemon in unwantedPokemons)
            {
                ...
                ColoredConsoleWrite(Brushes.White, $"Pokemon {pokemon.PokemonId} with {pokemon.Cp} CP has IV percent less than {keepPerfectPokemonLimit}%");
                ...
                if (pokemon.Favorite == 0)
                {
                    ...
                    {
                        ColoredConsoleWrite(Brushes.Magenta, $"Transferred {pokemonName} with {pokemon.Cp} CP");
                    }
                    else
                    {
                        var status = transferPokemonResponse.Status;

                        ColoredConsoleWrite(Brushes.Red, $"Somehow failed to transfer {pokemonName} with {pokemon.Cp} CP. " +
            ...
            for (var i = 0; i < dupes.Count(); i++)
            {
                for (var j = 0; j < dupes.ElementAt(i).Count() - 1; j++)
                {
                    ...
                    if (dubpokemon.Favorite == 0)
                    {
                        ...
                        ColoredConsoleWrite(Brushes.DarkGreen,
        ...
        private async Task TransferAllWeakPokemon(Client client, int cpThreshold)
        {
            ...
            PokemonId[] doNotTransfer = { // DO NOT EMPTY THIS ARRAY
                //PokemonId.Pidgey,
                //PokemonId.Rattata,
                //PokemonId.Weedle,
                //PokemonId.Zubat,
            ...
            //foreach (var unwantedPokemonType in unwantedPokemonTypes)
            {
                ...
                //                                          .ToList();
                ColoredConsoleWrite(Brushes.Gray, $"Grinding {pokemonToDiscard.Count} pokemon below {cpThreshold} CP.");
                await TransferAllGivenPokemons(client, pokemonToDiscard);

            }

            ColoredConsoleWrite(Brushes.Gray, $"Finished grinding all the meat");
        }
            ...
            foreach (var v in stats)
                if (v != null)
                {
                    ...
                    if (ClientSettings.LevelOutput == "time")
                        ColoredConsoleWrite(Brushes.Yellow, $"Current Level: " + v.Level + " (" + (v.Experience - XpDiff) + "/" + (v.NextLevelXp - XpDiff) + ")");
                    else if (ClientSettings.LevelOutput == "levelup")
                        if (Currentlevel != v.Level)
                        {
                            Currentlevel = v.Level;
                            ColoredConsoleWrite(Brushes.Magenta, $"Current Level: " + v.Level + ". XP needed for next Level: " + (v.NextLevelXp - v.Experience));
                        }
                }
        ...
        */
        ...
        {
            bot_started = true;
            startStopBotToolStripMenuItem.Header = "■ Stop Bot";
            Task.Run(() =>
            {
                try
                {
                ...
                {
                    ColoredConsoleWrite(Brushes.Red, "PTC Servers are probably down OR your credentials are wrong. Try google");
                }
                catch (Exception ex)
                {
                    ColoredConsoleWrite(Brushes.Red, $"Unhandled exception: {ex}");
                }
            });
        }
        ...
        public void stopBot()
        {
            ...
            {
                Stopping = true;
                ColoredConsoleWrite(Brushes.Red, $"Stopping the bot.. Waiting for the last action to be complete.");
            }
            else
            {
                ColoredConsoleWrite(Brushes.Red, $"An action is in play, please wait until it's done.");
            }
        }
        ...
        public void confirmBotStopped()
        {
            ...
            //pokestopsOverlay.Routes.Clear();
            //pokestopsOverlay.Markers.Clear();
            ColoredConsoleWrite(Brushes.Red, $"Bot successfully stopped.");
            startStopBotToolStripMenuItem.Header = "▶ Start Bot";
            Stopping = false;
            bot_started = false;
            pokeStops = null;
        }
        ...
        */
            ...
            if (client != null)
            {
                try
                {
                    ...
                    if (LuckyEgg != null)
                    {
                        ...
                        ColoredConsoleWrite(Brushes.Green, $"Using a Lucky Egg, we have {LuckyEgg.Count} left.");
                        ColoredConsoleWrite(Brushes.Yellow, $"Lucky Egg Valid until: {DateTime.Now.AddMinutes(30)}");

                        var stripItem = sender as MenuItem;
                        stripItem.IsEnabled = false;
                        await Task.Delay(30000);
                        stripItem.IsEnabled = true;
                    }
                    else
                    {
                        ColoredConsoleWrite(Brushes.Red, $"You don't have any Lucky Egg to use.");
                    }
                }
                catch (Exception ex)
                {
                    ColoredConsoleWrite(Brushes.Red, $"Unhandled exception in using lucky egg: {ex}");
                }
            }
            else
            {
                ColoredConsoleWrite(Brushes.Red, "Please start the bot before trying to use a lucky egg.");
            }
        }
        ...
        private async void forceUnbanToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (client != null && pokeStops != null)
            {
                if (ForceUnbanning)
                {
                    ColoredConsoleWrite(Brushes.Red, "A force unban attempt is in action... Please wait.");
                }
                else
                {
                    await ForceUnban(client);
            ...
            {
                ColoredConsoleWrite(Brushes.Red, "Please start the bot and wait for map to load before trying to force unban");
            }
        }
        ...
        private void InitializePokemonForm()
        {
            ...
            //            .First();
            //    args.Text = $"You have {family.Candy} {((PokemonId)((int)family.FamilyId))} Candy";
            //};
        }
        ...
        private async void ReloadPokemonList()
        {
            ...
            //}
            //catch (Exception ex) { ColoredConsoleWrite(Brushes.Red, ex.ToString()); client2 = null; }
        ...
        private void PokemonListButton_Click(object sender, CellClickEventArgs e)
        {
            ...
            catch (Exception ex) { ColoredConsoleWrite(Brushes.Red, ex.ToString()); client2 = null; ReloadPokemonList(); }
        }

        private async void TransferPokemon(PokemonData pokemon)
        {
            if (MessageBox.Show($"Are you sure you want to transfer {pokemon.PokemonId} with {pokemon.Cp} CP?", "Confirmation", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
            {
                var transferPokemonResponse = await client2.TransferPokemon(pokemon.Id);

                if (transferPokemonResponse.Status == 1)
                {
                    ColoredConsoleWrite(Brushes.Magenta, $"{pokemon.PokemonId} was transferred. {transferPokemonResponse.CandyAwarded} candy awarded");
                    ReloadPokemonList();
                }
                else
                {
                    ColoredConsoleWrite(Brushes.Magenta, $"{pokemon.PokemonId} could not be transferred");
                }
            }
        }
        ...
        private async void PowerUpPokemon(PokemonData pokemon)
        {
            ...
            {
                ColoredConsoleWrite(Brushes.Magenta, $"{pokemon.PokemonId} successfully upgraded.");
                ReloadPokemonList();
            }
            else
            {
                ColoredConsoleWrite(Brushes.Magenta, $"{pokemon.PokemonId} could not be upgraded");
            }
        }
        ...
        private async void EvolvePokemon(PokemonData pokemon)
        {
            ...
            if (evolvePokemonResponse.Result == 1)
            {
                ColoredConsoleWrite(Brushes.Magenta, $"{pokemon.PokemonId} successfully evolved into {evolvePokemonResponse.EvolvedPokemon.PokemonType}\n{evolvePokemonResponse.ExpAwarded} experience awarded\n{evolvePokemonResponse.CandyAwarded} candy awarded");
                ReloadPokemonList();
            }
            else
            {
                ColoredConsoleWrite(Brushes.Magenta, $"{pokemon.PokemonId} could not be evolved");
            }
        }
